...
def test_default_threshold_1000():
    diff = highlight_anomalies(_txns(), {})
    flagged_rows = {c.row for c in diff.changes}
    assert 1 in flagged_rows  # Rent 2100
    assert 2 in flagged_rows  # Salary 5200
    assert 0 not in flagged_rows  # Coffee 5
    assert 3 not in flagged_rows  # Lunch 12


def test_custom_threshold():
    diff = highlight_anomalies(_txns(), {"threshold": 3000})
    flagged_rows = {c.row for c in diff.changes}
    assert 2 in flagged_rows  # Salary 5200
    assert 1 not in flagged_rows  # Rent 2100 is under 3000


def test_flag_message_format():
    diff = highlight_anomalies(_txns(), {"threshold": 1000})
...
def test_no_anomalies_returns_empty():
    diff = highlight_anomalies(_txns(), {"threshold": 10000})
